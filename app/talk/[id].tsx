...
export default function TalkDetailScreen() {
      ...
      .on('postgres_changes', {
        ...
        try {
          ...
          const newBlock: Broadcast = {
            ...
            created_at: bc.created_at, block_order: bc.block_order, group_id: bc.group_id ?? null,
          }
          setGroups(prev => {
            if (bc.group_id) {
  ...
  const handleReplyPress = (group: BroadcastGroup) => {
    setReplyBroadcastId(group.anchorId)
    setReplyPreview(group.blocks.map(b => b.content).filter(c => c !== '　').join(' ').slice(0, 40) || '（画像）')
  }
  ...
  const handleSend = async () => {
    ...
    if (replyBroadcastId) {
      insertData.broadcast_id = replyBroadcastId
      ...
      <View style={styles.inputArea}>
        {replyBroadcastId && (
          <View style={styles.replyContext}>
            ...
            <Text style={styles.replyContextText} numberOfLines={1}>{replyPreview}</Text>
            <TouchableOpacity onPress={() => { setReplyBroadcastId(null); setReplyPreview('') }}>
        ...
        <View style={styles.inputRow}>
          <TextInput
            style={styles.input}
            placeholder={replyBroadcastId ? '配信にコメント...' : 'メッセージを送る...'}
...
const styles = StyleSheet.create({
  ...
  broadcastAvatar: {
    ...
    backgroundColor: Colors.button,
    alignItems: 'center', justifyContent: 'center',
    marginTop: 2, overflow: 'hidden', flexShrink: 0,
  },
  ...
  replyContextText: { flex: 1, fontSize: 12, color: Colors.accent, fontStyle: 'italic' },
  inputRow: {
